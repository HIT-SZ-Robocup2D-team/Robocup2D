...
#endif
	if(M_self.goalie() == true)              //仅守门员执行这个函数	
		updateMarkPairs(this);
}
...
*/
void
WorldModel::updateMarkPairs(WorldModel *wm)
{
    ...
    for( PlayerPtrCont::const_iterator it = wm->opponentsFromSelf().begin();  //遍历对手球员的位置，寻找在禁区内内的球员进行盯防
		it != wm->opponentsFromSelf().end(); 
		it++)
	{
		if ( our_penalty.contains ( (*it) -> pos(), 3.0 ) )
			markedOpp.push_back(*it);
	}
	
	 for( PlayerPtrCont::const_iterator it1 = wm->teammatesFromSelf().begin();  //遍历队友的位置，寻找在禁区内内的队友进行盯防
		it1 != wm->teammatesFromSelf().end(); 
		it1++)
	{
		if ( our_penalty.contains ( (*it1) -> pos(), 3.0 ) )
			markingTeammate.push_back(*it1);
	}
	...
	//计算距离并存储在矩阵中	
	int i = 0;    
	for( PlayerPtrCont::const_iterator it1 = markingTeammate.begin();
		 it1 != markingTeammate.end();
		 it1++, i++ )
	{
		int j = 0;
		for( PlayerPtrCont::const_iterator it2 = markedOpp.begin();
			 it2 != markedOpp.end();
			 it2++, j++ )
		{
			(*distance)[i][j] = -((*it1)-> pos().dist2((*it2)->pos()));        //因为要寻找最小距离匹配，所以全部取相反数
		}
	}
	...
	if( height > width )                                   //如果己方队友比较多，就从对手角度出发匹配
	{
		for( int i = 0; i < width; i++ )
			wm->findMax( *distance, i, width, maxLoc, 1, wm );
		//写入worldModel中的markPairs
		for( int i = 0; i < 10; i++ )
		{
			for( int j = 0; j < 10; j++ )
			{
				if( markingTeammate[maxLoc[i]]->unum() == wm->M_mark_pairs[j].first )    //这个时候maxLoc[i]记录盯防对手i的队友号码
					wm->M_mark_pairs[j].second = markedOpp[i]->unum();
			}
		}
	}
	...
	else
	{
		for( int i = 0; i < height; i++ )                      
			wm->findMax( *distance, i, height, maxLoc, 0, wm );
		//写入worldModel中的markPairs
		for( int i = 0; i < 10; i++ )
		{
			for( int j = 0; j < 10; j++ )
			{
				if( markingTeammate[i]->unum() == wm->M_mark_pairs[j].first )    //这个时候maxLoc[i]记录队友i盯防的对手号
					wm->M_mark_pairs[j].second = markedOpp[maxLoc[i]]->unum();
			}
		}
	}
}
...
void
WorldModel::findMax( std::vector <std::vector <double> > & vec, int i, int length, std::vector <int> & maxLoc, int option, WorldModel* wm )
{
	if( option == 0 )
	{
		int row = i;
		...
		for( int j = 0; j < row; j++ )
		{
			if( fabs(maxLoc[j] - maxLoc[row]) < EPS )
			{
				...
				wm->findMax( vec, temp, height, maxLoc, 0, wm );
			}
		}
	}
	...
	else
	{
		...
		for( int j = 0; j < col; j++ )
		{
			if( fabs(maxLoc[j] - maxLoc[col]) < EPS )
			{
				...
				wm->findMax( vec, temp, width, maxLoc, 1, wm );
			}
		}
	}
